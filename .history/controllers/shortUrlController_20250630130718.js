...
exports.createShortUrl = async (req, res, next) => {
  try {
    ...
    // if (custom_alias) {
    //   const existingAlias = await ShortUrl.findOne({ custom_alias });
    //   if (existingAlias) {
    //     return res.status(409).json({ error: 'Custom alias already exists' });
    //   }
    // }

    const short_code = custom_alias || generateShortCode(); // Generate short code if no alias is provided
    const shortUrl = new ShortUrl({ original_url, user_id, short_code, expires_at, metadata });
    await shortUrl.save();
